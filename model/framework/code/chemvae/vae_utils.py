...
class VAEUtils(object):
    def __init__(self,
                 exp_file='exp.json',
                 encoder_file="zinc_encoder.h5",
                 decoder_file="zinc_decoder.h5",
                 prop_pred_file="zinc_prop_pred.h5",
        ...
        if directory is not None:
            ...
            checkpt_path = os.path.join(filedir, "..", "..", "..", directory)
            exp_file = os.path.join(checkpt_path, exp_file)
        # load parameters
        self.params = hyperparameters.load_params(exp_file, False)
        if encoder_file is not None:
            self.params["encoder_weights_file"] = os.path.join(checkpt_path, encoder_file)
        if decoder_file is not None:
            self.params["decoder_weights_file"] = os.path.join(checkpt_path, decoder_file)
        self.params["prop_pred_weights_file"] = os.path.join(checkpt_path, prop_pred_file)
       
        # char stuff
        self.params['char_file'] = os.path.join(checkpt_path, 'zinc.json')
        chars = yaml.safe_load(open(self.params['char_file']))
        self.chars = chars
        self.params['NCHARS'] = len(chars)
        self.char_indices = dict((c, i) for i, c in enumerate(chars))
        self.indices_char = dict((i, c) for i, c in enumerate(chars))
        # encoder, decoder, property prediction
        ...
        self.params['data_file'] = os.path.join(checkpt_path, '250k_rndm_zinc_drugs_clean_3.csv')
